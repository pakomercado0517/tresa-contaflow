...
const createManualIncomeValidation = [
  ...
  body('fecha')
    ...
    .bail()
    .isISO8601()
    .withMessage('fecha debe tener un formato válido'),
  body('notes').optional().isString(),
  body('is_paid').optional().isBoolean().withMessage('el valor debe de ser de tipo booleano'),
  body('payment_date')
    .optional({ values: 'null' })
    .custom((value) => value === null || !isNaN(new Date(value).getTime()))
    .withMessage('payment_date debe ser true o false'),
];
...
const updateManualIncomeValidation = [
  ...
  body('fecha').optional().isISO8601().withMessage('fecha debe ser en un formato correcto'),
