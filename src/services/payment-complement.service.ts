...
export class PaymentComplementService {
   ...
   */
  async listForUser(params: ListPaymentComplementsParams): Promise<ListPaymentComplementsResult> {
    ...
    if (profiles.length === 0) {
      if (profileId) {
        throw new PaymentComplementServiceError("PROFILE_NOT_FOUND", "Perfil no encontrado");
      }
      return {
        data: [],
    ...
    const dateRange = this.getDateRangeFromMesAño(mes, año);
    if (dateRange) {
    ...
    const data = await this.buildListItems(links, profileById, dateRange);
  ...
  async getByIdForUser(
    ...
    const conciliation = await this.resolveConciliation(link.profile_id, items);

    const listBase = this.mapLinkToListItem(
      link,
      complement,
      profile,
      items,
    ...
    const itemResponses: PaymentComplementItemResponse[] = items.map((item) => {
      ...
      return {
        ...
        fecha_pago: this.toIsoString(item.fecha_pago),
  ...
  private async buildListItems(
    ...
    for (const profileId of profileIdsOnPage) {
      ...
      conciliationByProfile.set(
        profileId,
        await this.resolveConciliation(profileId, profileItems)
      );
    }
    ...
    return links.map((link) => {
      ...
      return this.mapLinkToListItem(link, complement, profile, items, conciliation, dateRange);
    });
  }

  private mapLinkToListItem(
    ...
    const totalPagado = itemsForSummary.reduce(
    ...
    const fechasPago = Array.from(
      new Set(
        itemsForSummary.map((item) => this.toIsoString(item.fecha_pago))
      )
    ).sort();
    ...
    return {
      ...
      fecha_emision: this.toIsoString(complement.fecha_emision),
  ...
  private async resolveConciliation(
    profileId: string,
    items: PaymentComplementItem[]
  ...
  private getDateRangeFromMesAño(
    mes?: number,
    año?: number
